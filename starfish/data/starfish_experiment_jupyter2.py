...
from starfish.image import Filter
ghp = Filter.GaussianHighPass(sigma=5) # test with 5
high_passed = ghp.run(dots, verbose=True, in_place=False)

glp = Filter.GaussianLowPass(sigma=0.5) # test with 0.5
low_passed = glp.run(high_passed, in_place=False, verbose=True)

masking_radius = 15
wth = Filter.WhiteTophat(masking_radius, is_volume=False)
wth_filtered = wth.run(low_passed, verbose=True, in_place=False)

dots_mp = low_passed.max_proj(Axes.ROUND, Axes.CH, Axes.ZPLANE)
dots_mp_numpy = dots_mp._squeezed_numpy(Axes.ROUND, Axes.CH, Axes.ZPLANE)
image(dots_mp_numpy)

images = [primary_image, nuclei]
masking_radius = 15
filt = Filter.WhiteTophat(masking_radius, is_volume=False)
filt.run(primary_image, verbose=True, in_place=True)
...
registration = Registration.FourierShiftRegistration(
    ...
    verbose=True)
registered_image = registration.run(primary_image, in_place=False)
registered_nuclei = registration.run(nuclei, in_place=False)
...
with warnings.catch_warnings():
    warnings.simplefilter("ignore")
    dots = low_passed.max_proj(Axes.ROUND, Axes.ZPLANE)
    dots_numpy = dots._squeezed_numpy(Axes.ROUND, Axes.ZPLANE)
    blobs_image = dots_numpy
    intensities = p.run(registered_image, blobs_image=blobs_image)
decoded = experiment.codebook.decode_per_round_max(intensities)
