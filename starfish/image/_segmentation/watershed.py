...
class Watershed(SegmentationAlgorithmBase):
    ...
    def run(self, primary_images: ImageStack, nuclei: ImageStack) -> np.ndarray:
        ...
        Parameters
        ----------
        primary_images : ImageStack PRIMARY SHOULD BE GFP
            contains primary image data
        nuclei : ImageStack DAPI
    ...
    def modified_run(self, primary_images: ImageStack, nuclei: ImageStack) -> (np.ndarray, np.ndarray):
        """Segments nuclei in 2-d using a nuclei ImageStack
        ...
        return label_image, sig_nuclei__mp_numpy

    def get_cell_num(self) -> int:
        # TODO make these parameterizable or determine whether they are useful or not
        if isinstance(self._segmentation_instance, _WatershedSegmenter):
            self._num_cells = self._segmentation_instance.num_cells
            return self._num_cells
        else:
            raise RuntimeError('Run segmentation before attempting to show results.')

    def show(self, figsize: Tuple[int, int]=(10, 10), filename: str=None) -> None:
        if isinstance(self._segmentation_instance, _WatershedSegmenter):
            self._segmentation_instance.show(figsize=figsize, filename=filename)
        else:
            raise RuntimeError('Run segmentation before attempting to show results.')

    def coordprint(self, label_image) -> None:
        if isinstance(self._segmentation_instance, _WatershedSegmenter):
            i = 0
            j = 0
            while i < 1023:
                while j < 1023:
                    ...
                    if label_image[i][j] != 0:
                        print(str(int(label_image[i][j])) + " at (" + str(i) + ", " + str(j) + ")")
                i = i+1
                j=0
        else:
            raise RuntimeError('Run segmentation before attempting to show results.')

    def coordreturn(self, label_image, x_pixels, y_pixels) -> dict:
        if isinstance(self._segmentation_instance, _WatershedSegmenter):
            ret_dict = {}
            cache=[0]
            i = 0
            ...
            while i < x_pixels:
                while j < y_pixels:
                    if label_image[i][j] not in cache:
                        cache.append(label_image[i][j])
                        index = str(int(label_image[i][j]))
                        ret_dict[index] = [j, i]
                    j = j+1
                i = i+1
                j=0
                if len(cache) == self._num_cells+1:
                    break
            return ret_dict
        else:
            raise RuntimeError('Run segmentation before attempting to show results.')
    ...
    def all_coords(self, label_image, x_pixels, y_pixels) -> dict:
        if isinstance(self._segmentation_instance, _WatershedSegmenter):
            ret_dict = {}
...
class _WatershedSegmenter:
    ...
    def show(self, figsize=(10, 10), filename: str=None):
